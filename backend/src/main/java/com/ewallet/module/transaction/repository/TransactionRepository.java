...
public interface TransactionRepository
    ...
    @Query("""
    SELECT 
        FUNCTION('MONTH', t.createdAt) AS txnMonth,
        SUM(CASE WHEN t.receiverUserId = :userId AND t.type = com.ewallet.module.transaction.enums.TransactionType.TOP_UP THEN t.amount ELSE 0 END) AS income,
        SUM(CASE WHEN t.senderUserId = :userId AND t.type IN (com.ewallet.module.transaction.enums.TransactionType.TRANSFER, com.ewallet.module.transaction.enums.TransactionType.WITHDRAW) THEN t.amount ELSE 0 END) AS expense
    FROM Transaction t
    WHERE (t.senderUserId = :userId OR t.receiverUserId = :userId)
      AND t.createdAt >= :startDate
    GROUP BY FUNCTION('MONTH', t.createdAt)
""")
    List<Object[]> getFinancialStatsGrouped(@Param("userId") Long userId, @Param("startDate") LocalDateTime startDate);
