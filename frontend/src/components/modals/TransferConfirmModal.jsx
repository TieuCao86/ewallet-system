    ...
    <Modal isOpen={isOpen} onClose={onClose} title={transferOtpStep ? 'Xác thực OTP chuyển tiền' : 'Xác nhận chuyển tiền'}>
      {!transferOtpStep ? (
        ...
        <form onSubmit={handleVerifyTransferOtp}>
          <p style={{ color: 'var(--muted)', fontSize: '0.88rem', margin: '0 0 20px', lineHeight: 1.5 }}>
            Để đảm bảo an toàn cho tài khoản ví, vui lòng xác nhận mã OTP gửi qua SMS tới số điện thoại <strong>{userProfile.phone}</strong> để hoàn tất chuyển <strong>{displayAmount}đ</strong> tới <strong>{transferPhone}</strong>.
          </p>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end', marginBottom: '16px' }}>
            ...
            >
              {transferCountdown > 0 ? `Gửi lại (${transferCountdown}s)` : 'Gửi mã'}
            </button>
          </div>
